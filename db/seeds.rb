...
['juan', 'andrea', 'leon', 'andres', 'natalia', 'camilo', 'rusbel', 'johan'].each do |name|
    User.create email: "#{name}@bar.com", password: '123456'
end
...
owner = User.find_by(email: 'foo@bar.com')
...
['conceptualización', 'Conclusiones del curso', ['juan:1', 'leon:2', 'andrea:random']],].each do |category, description, participant_set|
    ...
    participants = participant_set.map do |participant|
        ...
        user: User.find_by(email: "#{user_name}@bar.com"),
